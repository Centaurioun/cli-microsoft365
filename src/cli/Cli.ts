...
export class Cli {
  ...
  public static helpModes: string[] = [
    "options",
    "examples",
    "remarks",
    "response",
  ...
  public getSettingWithDefaultValue<TValue>(
    settingName: string,
    defaultValue: TValue,
  ): TValue {
  ...
  public async execute(
    ...
    if (this.currentCommandName) {
      for (let i = 0; i < this.commands.length; i++) {
        ...
        if (
          command.name === this.currentCommandName ||
          (command.aliases &&
    ...
    if (this.commandToExecute) {
      ...
      try {
        this.optionsFromArgs = {
          options: this.getCommandOptionsFromArgs(
            rawArgs,
            this.commandToExecute,
          ),
        };
      } catch (e: any) {
        const optionsWithoutShorts = Cli.removeShortOptions({
          options: parsedArgs,
        });
    ...
    if (!this.commandToExecute || showHelp || parsedArgs.h || parsedArgs.help) {
    ...
    try {
      ...
      const contextCommandOptions = this.loadOptionsFromContext(
        this.commandToExecute.options,
        optionsWithoutShorts.options.debug,
      );
      optionsWithoutShorts.options = {
    ...
    if (optionsWithoutShorts.options.output === undefined) {
      optionsWithoutShorts.options.output = this.getSettingWithDefaultValue<
        string | undefined
      >(settingsNames.output, "json");
    }

    const validationResult = await this.commandToExecute.command.validate(
      optionsWithoutShorts,
      this.commandToExecute,
    );
    ...
    return Cli.executeCommand(
      ...
      (_) => process.exit(0),
      (err) => this.closeWithError(err, optionsWithoutShorts),
    );
  }

  public static async executeCommand(
    command: Command,
    args: { options: minimist.ParsedArgs },
  ): Promise<void> {
    const logger: Logger = {
    ...
    if (args.options.debug) {
      logger.logToStderr(
        `Executing command ${command.name} with options ${JSON.stringify(
          args,
        )}`,
      );
    }
    ...
    const showSpinner =
      cli.getSettingWithDefaultValue<boolean>(
        settingsNames.showSpinner,
        true,
      ) && args.options.output !== "none";
  ...
  public static async executeCommandWithOutput(
    ...
    const logger: Logger = {
      log: (message: any): void => {
        const formattedMessage = Cli.formatOutput(
          command,
          message,
          args.options,
        );
    ...
    if (args.options.debug && args.options.output !== "none") {
      const message = `Executing command ${
        command.name
      } with options ${JSON.stringify(args)}`;
      if (listener?.stderr) {
        listener.stderr(message);
      }
      logErr.push(message);
  ...
  public loadAllCommands(): void {
    ...
    files.forEach((file) => {
      if (
        file.indexOf(`${path.sep}commands${path.sep}`) > -1 &&
  ...
  public loadCommandFromArgs(commandNameWords: string[]): void {
    ...
    const isCompletionCommand: boolean =
      commandNameWords.indexOf("completion") > -1;
    ...
    if (commandNameWords.length === 1) {
      commandFilePath = path.join(
        this.commandsFolder,
        "commands",
        `${commandNameWords[0]}.js`,
      );
    } else {
      if (commandNameWords.length === 2) {
        ...
        commandFilePath = path.join(
          ...
          "commands",
          commandNameWords[1],
          commandNameWords.slice(1).join("-") + ".js",
        );
      }
    }
  ...
  private loadOptionsFromContext(
  ...
  private getCommandOptionsFromArgs(
    args: string[],
    ...
    if (commandInfo) {
      ...
      if (commandTypes) {
        ...
        minimistOptions.boolean = commandTypes.boolean.filter((optionName) =>
          args.some(
            (arg) => `--${optionName}` === arg || `-${optionName}` === arg,
          ),
        );
      }
  ...
  private getRewrittenArgs(
    ...
    return args.map((arg: string, index: number, array: string[]) => {
      ...
      if (
        booleanTypes.some(
          (t) => `--${t}` === array[index - 1] || `-${t}` === array[index - 1],
        )
      ) {
        ...
        if (!validation.isValidBoolean(rewrittenBoolean)) {
          ...
          throw new Error(
            `The value '${arg}' for option '${optionName}' is not a valid boolean`,
          );
        }
  ...
  private static formatOutput(
    ...
    const originalObject: any = Array.isArray(logStatement)
      ? Cli.getFirstNonUndefinedArrayItem(logStatement)
      : logStatement;
    const originalProperties: string[] =
      originalObject && typeof originalObject !== "string"
    ...
    if (this.shouldTrimOutput(options.output)) {
      ...
      if (
        arrayType === "object" &&
        currentCommand &&
        currentCommand.defaultProperties
      ) {
        ...
        if (
          ...
          JSON.stringify(Object.getOwnPropertyNames(logStatement[0]))
        ) {
          ...
          if (logStatement[0].value && Array.isArray(logStatement[0].value)) {
            logStatement = logStatement[0].value;
          }

          logStatement = logStatement.map((s: any) =>
            formatting.filterObject(
              s,
              currentCommand.defaultProperties as string[],
            ),
          );
        }
      }
  ...
  private printCommandHelp(helpMode: string): void {
    ...
    if (commandNameWords.length === 1) {
      ...
      if (commandNameWords.length === 2) {
        pathChunks.push(
          commandNameWords[0],
          `${commandNameWords.join("-")}.mdx`,
        );
      } else {
        pathChunks.push(
          commandNameWords[0],
          commandNameWords[1],
          commandNameWords.slice(1).join("-") + ".mdx",
        );
      }
    }
    ...
    if (fs.existsSync(helpFilePath)) {
      ...
      helpContents = md.md2plain(
        helpContents,
        path.join(this.commandsFolder, "..", "..", "docs"),
      );
      Cli.log();
  ...
  private getHelpMode(options: any): string {
    ...
    if (h || help) {
      ...
      if (typeof helpMode === "boolean" || typeof helpMode !== "string") {
        ...
        return this.getSettingWithDefaultValue<string>(
          settingsNames.helpMode,
          Cli.defaultHelpMode,
        );
      } else {
        ...
        if (Cli.helpModes.indexOf(lowerCaseHelpMode) < 0) {
          Cli.getInstance().closeWithError(
            `Unknown help mode ${helpMode}. Allowed values are ${Cli.helpModes.join(
              ", ",
            )}`,
            { options },
            false,
  ...
  private getHelpSection(helpMode: string, helpContents: string): string {
    ...
    const titleAndUsage = helpContents.substring(
      0,
      helpContents.indexOf("## Options"),
    );
  ...
  private printAvailableCommands(): void {
    ...
    getCommandsForGroup();
    if (
      Object.keys(commandsToPrint).length === 0 &&
    ...
    if (namesOfCommandsToPrint.length > 0) {
      ...
      const maxLength: number =
        Math.max(...namesOfCommandsToPrint.map((s) => s.length)) + 10;
      ...
      for (const commandName in commandsToPrint) {
        Cli.log(
          `  ${`${commandName} [options]`.padEnd(maxLength, " ")}  ${
            commandsToPrint[commandName].command.description
          }`,
        );
      }
    }

    const namesOfCommandGroupsToPrint: string[] =
      Object.keys(commandGroupsToPrint);
    if (namesOfCommandGroupsToPrint.length > 0) {
      ...
      const maxLength: number =
        Math.max(...namesOfCommandGroupsToPrint.map((s) => s.length)) + 2;

      Cli.log(`Commands groups:`);
      Cli.log();

      // sort commands groups (because of aliased commands)
      const sortedCommandGroupsToPrint = Object.keys(commandGroupsToPrint)
        .sort()
      ...
      for (const commandGroup in sortedCommandGroupsToPrint) {
        Cli.log(
          `  ${`${commandGroup} *`.padEnd(maxLength, " ")}  ${
            commandGroupsToPrint[commandGroup]
          } command${commandGroupsToPrint[commandGroup] === 1 ? "" : "s"}`,
        );
      }
    }
  ...
  private closeWithError(
    error: any,
    args: CommandArgs,
    showHelpIfEnabled: boolean = false,
  ): void {
    ...
    if (
      (!args.options.output || args.options.output === "json") &&
      !this.getSettingWithDefaultValue<boolean>(
        settingsNames.printErrorsAsPlainText,
        true,
      )
    ) {
    ...
    if (
      showHelpIfEnabled &&
      this.getSettingWithDefaultValue<boolean>(
        settingsNames.showHelpOnFailure,
        showHelpIfEnabled,
      )
    ) {
  ...
  private static error(message?: any, ...optionalParams: any[]): void {
    ...
    const errorOutput: string = cli.getSettingWithDefaultValue(
      settingsNames.errorOutput,
      "stderr",
    );
  ...
  public static async handleMultipleResultsFound(
    promptMessage: string,
    errorMessage: string,
    values: { [key: string]: object },
  ): Promise<object | CommandError> {
    const prompt: boolean =
  ...
  private static removeShortOptions(args: { options: minimist.ParsedArgs }): {
    options: minimist.ParsedArgs;
  } {
    const filteredArgs = JSON.parse(JSON.stringify(args));
    const optionsToRemove: string[] = Object.getOwnPropertyNames(
      args.options,
  ...
  private static loadOptionValuesFromFiles(args: {
    options: minimist.ParsedArgs;
  }): void {
