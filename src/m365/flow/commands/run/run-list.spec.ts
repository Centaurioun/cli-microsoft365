...
describe(commands.RUN_LIST, () => {
  ...
  it("retrieves all runs for a specific flow", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://management.azure.com/providers/Microsoft.ProcessSimple/environments/${environmentName}/flows/${flowName}/runs?api-version=2016-11-01`
      ) {
        if (
          opts.headers?.accept &&
  ...
  it("retrieves all runs for a specific flow as admin", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://management.azure.com/providers/Microsoft.ProcessSimple/scopes/admin/environments/${environmentName}/flows/${flowName}/runs?api-version=2016-11-01`
      ) {
        if (
          opts.headers?.accept &&
          (opts.headers.accept as string).indexOf("application/json") === 0
  ...
  it("retrieves all runs with a specific status for a specific flow", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://management.azure.com/providers/Microsoft.ProcessSimple/environments/${environmentName}/flows/${flowName}/runs?api-version=2016-11-01&$filter=status eq '${status}'`
      ) {
        if (
          opts.headers?.accept &&
          (opts.headers.accept as string).indexOf("application/json") === 0
  ...
  it("retrieves all runs between two dates for a specific flow", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://management.azure.com/providers/Microsoft.ProcessSimple/environments/${environmentName}/flows/${flowName}/runs?api-version=2016-11-01&$filter=startTime ge ${triggerStartTime} and startTime lt ${triggerEndTime}`
      ) {
        if (
          opts.headers?.accept &&
          (opts.headers.accept as string).indexOf("application/json") === 0
  ...
  it("correctly handles no environment found", async () => {
    ...
    await assert.rejects(
      command.action(logger, {
        options: { environmentName: environmentName, flowName: flowName },
      } as any),
      new CommandError(
        `Access to the environment '${environmentName}' is denied.`,
      ),
    );
  });
  ...
  it("correctly handles no runs for this flow found", async () => {
    ...
    await command.action(logger, {
      options: {
        verbose: true,
        environmentName: "Default-48595cc3-adce-4267-8e99-0c838923dbb9",
        flowName: "16c90c26-25e0-4800-8af9-da594e02d427",
      },
    });
  ...
  it("fails validation if the flowName is not a valid GUID", async () => {
    const actual = await command.validate(
      { options: { environmentName: environmentName, flowName: "invalid" } },
      commandInfo,
    );
