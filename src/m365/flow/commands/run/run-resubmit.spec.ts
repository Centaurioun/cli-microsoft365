...
describe(commands.RUN_RESUBMIT, () => {
  ...
  afterEach(() => {
    sinonUtil.restore([request.post, request.get, Cli.prompt]);
  });
  ...
  it("correctly handles no environment found when prompt confirmed", async () => {
    sinon.stub(request, "get").rejects({
      error: {
        ...
        message:
          "You are not permitted to make flows in this 'Default-eff8592e-e14a-4ae8-8771-d96d5c549e1c'. Please switch to the default environment, or to one of your own environment(s), where you have maker permissions.",
      },
    });
  ...
  it("correctly handles specified Microsoft Flow not found when prompt confirmed", async () => {
    sinon.stub(request, "get").rejects({
      error: {
        ...
        message:
          "The caller with object id 'da8f7aea-cf43-497f-ad62-c2feae89a194' does not have permission for connection '0f64d9dd-01bb-4c1b-95b3-cb4a1a08ac88' under Api 'shared_logicflows'.",
      },
    });
  ...
  it("correctly handles specified Microsoft Flow run not found when prompt confirmed", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        (opts.url as string).indexOf(
          `providers/Microsoft.ProcessSimple/environments/Default-d87a7535-dd31-4437-bfe1-95340acd55c6/flows/0f64d9dd-01bb-4c1b-95b3-cb4a1a08ac72/triggers?api-version=2016-11-01`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
    ...
    await assert.rejects(
      ...
      new CommandError(
        `Request to Azure Resource Manager failed with error: '{"error":{"code":"WorkflowRunNotFound","message":"The workflow '0f64d9dd-01bb-4c1b-95b3-cb4a1a08ac72' run '08585981115186985105550762688CP233' could not be found."}}`,
      ),
    );
  });
  ...
  it("correctly getting triggername for the specified Microsoft Flow when prompt confirmed (debug)", async () => {
    const getStub = sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        (opts.url as string).indexOf(
          `providers/Microsoft.ProcessSimple/environments/Default-d87a7535-dd31-4437-bfe1-95340acd55c6/flows/0f64d9dd-01bb-4c1b-95b3-cb4a1a08ac88/triggers?api-version=2016-11-01`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
    ...
    const postStub = sinon.stub(request, "post").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://management.azure.com/providers/Microsoft.ProcessSimple/environments/Default-d87a7535-dd31-4437-bfe1-95340acd55c6/flows/0f64d9dd-01bb-4c1b-95b3-cb4a1a08ac88/triggers/manual/histories/08585981115186985105550762687CU161/resubmit?api-version=2016-11-01`
      ) {
    ...
    assert.notStrictEqual(
      loggerLogToStderrSpy
        .getCall(1)
        .args[0].indexOf("Retrieved trigger: manual"),
      -1,
    );
  ...
  it("resubmits the specified Microsoft Flow run when confirm specified (debug)", async () => {
    ...
    assert.strictEqual(
      ...
      "https://management.azure.com/providers/Microsoft.ProcessSimple/environments/Default-d87a7535-dd31-4437-bfe1-95340acd55c6/flows/0f64d9dd-01bb-4c1b-95b3-cb4a1a08ac88/triggers?api-version=2016-11-01",
    );
    assert.strictEqual(
      postStub.lastCall.args[0].url,
      "https://management.azure.com/providers/Microsoft.ProcessSimple/environments/Default-d87a7535-dd31-4437-bfe1-95340acd55c6/flows/0f64d9dd-01bb-4c1b-95b3-cb4a1a08ac88/triggers/manual/histories/08585981115186985105550762687CU161/resubmit?api-version=2016-11-01",
    );
  });
