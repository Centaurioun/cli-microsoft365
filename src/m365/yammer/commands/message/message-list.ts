...
class YammerMessageListCommand extends YammerCommand {
  private items: any[];
  private static readonly feedTypes: string[] = [
    "All",
    "Top",
    "My",
    "Following",
  ...
  #initValidators(): void {
    this.validators.push(async (args: CommandArgs) => {
      ...
      if (
        args.options.feedType &&
        (args.options.groupId || args.options.threadId)
      ) {
      ...
      if (
        args.options.feedType &&
        YammerMessageListCommand.feedTypes.indexOf(args.options.feedType) < 0
      ) {
        return `${
          args.options.feedType
        } is not a valid value for the feedType option. Allowed values are ${YammerMessageListCommand.feedTypes.join(
          "|",
        )}`;
      }

      if (
        args.options.olderThanId &&
        typeof args.options.olderThanId !== "number"
      ) {
