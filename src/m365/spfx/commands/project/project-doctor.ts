...
class SpfxProjectDoctorCommand extends BaseProjectCommand {
  ...
  #initValidators(): void {
    this.validators.push(async (args: CommandArgs) => {
      if (args.options.packageManager) {
        if (
          SpfxProjectDoctorCommand.packageManagers.indexOf(
            args.options.packageManager,
          ) < 0
        ) {
          return `${
            args.options.packageManager
          } is not a supported package manager. Supported package managers are ${SpfxProjectDoctorCommand.packageManagers.join(
            ", ",
          )}`;
        }
      }
  ...
  public async commandAction(logger: Logger, args: CommandArgs): Promise<void> {
    ...
    if (this.projectRootPath === null) {
      throw new CommandError(
        ...
        SpfxProjectDoctorCommand.ERROR_NO_PROJECT_ROOT_FOLDER,
      );
    }
    ...
    if (!project.version) {
      throw new CommandError(
        ...
        SpfxProjectDoctorCommand.ERROR_NO_VERSION,
      );
    }

    if (!this.supportedVersions.includes(project.version)) {
      throw new CommandError(
        `CLI for Microsoft 365 doesn't support validating projects built using SharePoint Framework v${project.version}`,
        SpfxProjectDoctorCommand.ERROR_UNSUPPORTED_VERSION,
      );
    }
  ...
  private getTextReport(findings: FindingToReport[]): string {
    ...
    const s: string[] = [
      ...
      "-----------------------",
      os.EOL,
      reportData.packageManagerCommands.join(os.EOL),
      os.EOL,
      os.EOL,
    ];
...
`;
    ...
    const s: string[] = [
      `# Validate project ${path.basename(this.projectRootPath as string)}`,
      os.EOL,
      os.EOL,
  ...
  private getTourReport(findings: FindingToReport[]): string {
    const tourFindings: FindingTour = {
      title: `Validate project ${path.basename(
        this.projectRootPath as string,
      )}`,
      steps: [],
    };
