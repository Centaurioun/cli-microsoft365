...
class SpfxProjectUpgradeCommand extends BaseProjectCommand {
  ...
  #initTelemetry(): void {
    this.telemetry.push((args: CommandArgs) => {
      ...
      // uncomment to support upgrading to preview releases
      if (
        prerelease(this.telemetryProperties.toVersion) &&
        !args.options.preview
      ) {
        this.telemetryProperties.toVersion =
          this.supportedVersions[this.supportedVersions.length - 2];
      }
    });
  }
  ...
  #initValidators(): void {
    this.validators.push(async (args: CommandArgs) => {
      if (args.options.packageManager) {
        if (
          SpfxProjectUpgradeCommand.packageManagers.indexOf(
            args.options.packageManager,
          ) < 0
        ) {
          return `${
            args.options.packageManager
          } is not a supported package manager. Supported package managers are ${SpfxProjectUpgradeCommand.packageManagers.join(
            ", ",
          )}`;
        }
      }
      ...
      if (args.options.shell) {
        if (SpfxProjectUpgradeCommand.shells.indexOf(args.options.shell) < 0) {
          return `${
            args.options.shell
          } is not a supported shell. Supported shells are ${SpfxProjectUpgradeCommand.shells.join(
            ", ",
          )}`;
        }
      }
  ...
  public async commandAction(logger: Logger, args: CommandArgs): Promise<void> {
    ...
    if (this.projectRootPath === null) {
      throw new CommandError(
        ...
        SpfxProjectUpgradeCommand.ERROR_NO_PROJECT_ROOT_FOLDER,
      );
    }
    ...
    if (this.supportedVersions.indexOf(this.toVersion) < 0) {
      throw new CommandError(
        `CLI for Microsoft 365 doesn't support upgrading SharePoint Framework projects to version ${
          this.toVersion
        }. Supported versions are ${this.supportedVersions.join(", ")}`,
        SpfxProjectUpgradeCommand.ERROR_UNSUPPORTED_TO_VERSION,
      );
    }
    ...
    if (!this.projectVersion) {
      throw new CommandError(
        `Unable to determine the version of the current SharePoint Framework project`,
        SpfxProjectUpgradeCommand.ERROR_NO_VERSION,
      );
    }
    ...
    if (pos < 0) {
      throw new CommandError(
        `CLI for Microsoft 365 doesn't support upgrading projects built using SharePoint Framework v${this.projectVersion}`,
        SpfxProjectUpgradeCommand.ERROR_UNSUPPORTED_FROM_VERSION,
      );
    }
    ...
    if (pos > posTo) {
      throw new CommandError(
        "You cannot downgrade a project",
        SpfxProjectUpgradeCommand.ERROR_NO_DOWNGRADE,
      );
    }
    ...
    const versionsToUpgradeTo: string[] = this.supportedVersions
      .slice(pos + 1, posTo + 1)
      .reverse();
    try {
    ...
    // dedupe
    const findings: Finding[] = this.allFindings.filter(
      (f: Finding, i: number) => {
        const firstFindingPos: number = this.allFindings.findIndex(
          (f1) => f1.id === f.id,
        );
    ...
    // flatten
    const findingsToReport: FindingToReport[] = (
      [] as FindingToReport[]
    ).concat.apply(
      [],
      findings.map((f) => {
    ...
    findingsToReport.forEach((f) => {
      ...
      if (f.resolution.startsWith("uninstallDev")) {
        f.resolution = f.resolution.replace(
          "uninstallDev",
          packageManager.getPackageManagerCommand(
            "uninstallDev",
            this.packageManager,
          ),
        );
      ...
      if (f.resolution.startsWith("copy_cmd")) {
        f.resolution = f.resolution.replace(
          "copy_cmd",
          fsUtil.getCopyCommand("copyCommand", this.shell),
        );
        f.resolution = f.resolution.replace(
          "DestinationParam",
          fsUtil.getCopyCommand("copyDestinationParam", this.shell),
        );
        return;
      }
      ...
      if (f.resolution.startsWith("create_dir_cmd")) {
        f.resolution = f.resolution.replace(
          "create_dir_cmd",
          fsUtil.getDirectoryCommand("createDirectoryCommand", this.shell),
        );
        f.resolution = f.resolution.replace(
          "NameParam",
          fsUtil.getDirectoryCommand("createDirectoryNameParam", this.shell),
        );
        f.resolution = f.resolution.replace(
          "PathParam",
          fsUtil.getDirectoryCommand("createDirectoryPathParam", this.shell),
        );
        f.resolution = f.resolution.replace(
          "ItemTypeParam",
          fsUtil.getDirectoryCommand(
      ...
      if (f.resolution.startsWith("add_cmd")) {
        const pathStart: number =
          f.resolution.indexOf("[BEFOREPATH]") + "[BEFOREPATH]".length;
        ...
        const contentStart: number =
          f.resolution.indexOf("[BEFORECONTENT]") + "[BEFORECONTENT]".length;
        const contentEnd: number = f.resolution.indexOf("[AFTERCONTENT]");
        const fileContent: string = f.resolution.substring(
          contentStart,
          contentEnd,
        );
      ...
      if (f.resolution.startsWith("remove_cmd")) {
        f.resolution = f.resolution.replace(
          "remove_cmd",
          fsUtil.getRemoveCommand("removeFileCommand", this.shell),
        );
        return;
      }
    });
    ...
    switch (args.options.output) {
      ...
      case "json":
        ...
        break;
      case "tour":
        this.writeReportTourFolder(
          this.getTourReport(findingsToReport, project),
        );
        break;
      case "md":
  ...
  private writeReportTourFolder(findingsToReport: any): void {
    const toursFolder: string = path.join(
      this.projectRootPath as string,
      ".tours",
    );
    ...
    const tourFilePath: string = path.join(
      this.projectRootPath as string,
      ".tours",
      "upgrade.tour",
    );
  ...
  private getTextReport(findings: FindingToReport[]): string {
    ...
    const s: string[] = [
      ...
      reportData.packageManagerCommands
        .concat(
          reportData.commandsToExecute.filter(
            (command) =>
              command.indexOf(
                packageManager.getPackageManagerCommand(
                  "install",
      ...
      os.EOL,
      os.EOL,
      Object.keys(reportData.modificationPerFile)
        .map((file) => {
          return [
            file,
            os.EOL,
            "-".repeat(file.length),
            os.EOL,
            reportData.modificationPerFile[file]
              .map(
                (m: ReportDataModification) =>
                  `${m.description}:${os.EOL}${m.modification}${os.EOL}`,
              )
              .join(os.EOL),
            os.EOL,
          ].join("");
...
`;
      ...
      findingsToReport.push(
        ...
        os.EOL,
        `File: [${f.file}${
          f.position ? `:${f.position.line}:${f.position.character}` : ""
        }](${f.file})`,
        os.EOL,
        os.EOL,
      );
    ...
    const s: string[] = [
      `# Upgrade project ${path.basename(this.projectRootPath as string)} to v${
        this.toVersion
      }`,
      os.EOL,
      os.EOL,
      ...
      os.EOL,
      os.EOL,
      `Following is the list of steps required to upgrade your project to SharePoint Framework version ${this.toVersion}. [Summary](#Summary) of the modifications is included at the end of the report.`,
      os.EOL,
      os.EOL,
        ...
        .map((file) => {
          return [
            ...
            reportData.modificationPerFile[file]
              .map(
                (m: ReportDataModification) =>
                  `${m.description}:${os.EOL}${os.EOL}\`\`\`${reportData.modificationTypePerFile[file]}${os.EOL}${m.modification}${os.EOL}\`\`\``,
              )
              .join(os.EOL + os.EOL),
            os.EOL,
          ].join("");
  ...
  private getTourReport(findings: FindingToReport[], project: Project): string {
    const tourFindings: FindingTour = {
      title: `Upgrade project ${path.basename(
        this.projectRootPath as string,
      )} to v${this.toVersion}`,
      steps: [],
    };
    ...
    findings.forEach((f) => {
      ...
      let file: string | undefined = fs.existsSync(
        path.join(project.path, f.file),
      )
        ? f.file
        : undefined;
  ...
  private getReportData(findings: FindingToReport[]): ReportData {
    ...
    const packageManagerCommands: string[] =
      packageManager.reducePackageManagerCommand({
        packagesDepExact,
        packagesDevExact,
        packagesDepUn,
        packagesDevUn,
    ...
    if (this.packageManager === "npm") {
      const dedupeFinding: FindingToReport[] = findings.filter(
        (f) => f.id === "FN017001",
      );
