...
export abstract class BaseProjectCommand extends AnonymousCommand {
  ...
  protected getProject(projectRootPath: string): Project {
    ...
    if (fs.existsSync(configJsonPath)) {
      try {
        const source = formatting.removeSingleLineComments(
    ...
    const copyAssetsJsonPath: string = path.join(
      projectRootPath,
    ...
    if (fs.existsSync(copyAssetsJsonPath)) {
      try {
        const source = formatting.removeSingleLineComments(
          fs.readFileSync(copyAssetsJsonPath, "utf-8"),
        );
    ...
    const deployAzureStorageJsonPath: string = path.join(
      projectRootPath,
    ...
    if (fs.existsSync(deployAzureStorageJsonPath)) {
      try {
        const source = formatting.removeSingleLineComments(
          fs.readFileSync(deployAzureStorageJsonPath, "utf-8"),
        );
    ...
    const packageSolutionJsonPath: string = path.join(
      projectRootPath,
    ...
    if (fs.existsSync(packageSolutionJsonPath)) {
      try {
        const source = formatting.removeSingleLineComments(
          fs.readFileSync(packageSolutionJsonPath, "utf-8"),
        );
    ...
    const serveJsonPath: string = path.join(
      projectRootPath,
      "config/serve.json",
    );
    if (fs.existsSync(serveJsonPath)) {
      try {
        const source = formatting.removeSingleLineComments(
          fs.readFileSync(serveJsonPath, "utf-8"),
        );
    ...
    const tsConfigJsonPath: string = path.join(
      projectRootPath,
      "tsconfig.json",
    );
    ...
    const tsLintJsonPath: string = path.join(
      projectRootPath,
      "config/tslint.json",
    );
    if (fs.existsSync(tsLintJsonPath)) {
      try {
        const source = formatting.removeSingleLineComments(
          fs.readFileSync(tsLintJsonPath, "utf-8"),
        );
    ...
    const tsLintJsonRootPath: string = path.join(
      projectRootPath,
      "tslint.json",
    );
    if (fs.existsSync(tsLintJsonRootPath)) {
      try {
        const source = formatting.removeSingleLineComments(
          fs.readFileSync(tsLintJsonRootPath, "utf-8"),
        );
    ...
    const writeManifestJsonPath: string = path.join(
      projectRootPath,
    ...
    if (fs.existsSync(writeManifestJsonPath)) {
      try {
        const source = formatting.removeSingleLineComments(
          fs.readFileSync(writeManifestJsonPath, "utf-8"),
        );
    ...
    if (fs.existsSync(yoRcJsonPath)) {
      try {
        const source = formatting.removeSingleLineComments(
          fs.readFileSync(yoRcJsonPath, "utf-8"),
        );
    ...
    project.vsCode = {};
    const vsCodeSettingsPath: string = path.join(
      projectRootPath,
      ".vscode",
      "settings.json",
    );
    ...
    const vsCodeExtensionsPath: string = path.join(
      projectRootPath,
      ".vscode",
      "extensions.json",
    );
    ...
    const vsCodeLaunchPath: string = path.join(
      projectRootPath,
      ".vscode",
      "launch.json",
    );
    if (fs.existsSync(vsCodeLaunchPath)) {
      try {
        const source = formatting.removeSingleLineComments(
          fs.readFileSync(vsCodeLaunchPath, "utf-8"),
        );
    ...
    const srcFiles: string[] = fsUtil.readdirR(
    ...
    const manifests: Manifest[] = manifestFiles.map((f) => {
      const manifestStr = formatting.removeSingleLineComments(
    ...
    const tsFiles: string[] = srcFiles.filter(
  ...
  protected getProjectVersion(): string | undefined {
    const yoRcPath: string = path.resolve(
