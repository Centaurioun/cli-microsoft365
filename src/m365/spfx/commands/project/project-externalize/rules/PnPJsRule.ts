...
export class PnPJsRule extends BasicDependencyRule {
  private pnpModules = [
    {
      ...
      globalDependencies: ["@pnp/common", "@pnp/logging", "tslib"],
  ...
  public visit(project: Project): Promise<VisitationResult> {
    ...
    const rawFileEdits: FileEdit[][] = this.pnpModules
      .filter((x) => findings.find((y) => y.key === x.key) !== undefined)
    ...
    const fileEdits: FileEdit[] =
      rawFileEdits.length > 0
        ? rawFileEdits.reduce((x, y) => [...x, ...y])
        : [];
    if (findings.filter((x) => x.key && x.key !== "@pnp/pnpjs").length > 0) {
      // we're adding tslib only if we found other packages that are not the bundle which already contains tslib
      findings.push({
        key: "tslib",
        globalName: "tslib",
  ...
  private getEntryFilesList(project: Project): string[] {
    return project?.manifests
      ? project.manifests.map((x) => x.path.replace(".manifest.json", ".ts"))
      : [];
  }

  private getModuleAndParents(
    project: Project,
    moduleName: string,
  ): ExternalizeEntry[] {
    const result: ExternalizeEntry[] = [];
    const moduleConfiguration = this.pnpModules.find(
      (x) => x.key === moduleName,
    );

    if (project.packageJson && moduleConfiguration) {
      const version: string | undefined =
        project.packageJson.dependencies?.[moduleName];
      ...
      if (version) {
        result.push({
          ...
          }@${version}/dist/${moduleName.replace("@pnp/", "")}.es5.umd${
            moduleName === "@pnp/common" || moduleName === " @pnp/pnpjs"
              ? ".bundle"
              : ""
          }.min.js`,
        });
