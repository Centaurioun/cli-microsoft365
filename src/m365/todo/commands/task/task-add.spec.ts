...
describe(commands.TASK_ADD, () => {
  ...
  beforeEach(() => {
    ...
    postStub = sinon.stub(request, "post").callsFake(async (opts: any) => {
      if (
        opts.url ===
        "https://graph.microsoft.com/v1.0/me/todo/lists/AQMkADlhMTRkOGEzLWQ1M2QtNGVkNS04NjdmLWU0NzJhMjZmZWNmMwAuAAADKvwNgAMNPE_zFNRJXVrU1wEAhHKQZHItDEOVCn8U3xuA2AABmQeVPwAAAA==/tasks"
      ) {
  ...
  afterEach(() => {
    sinonUtil.restore([request.get, request.post, Date.now]);
  });
  ...
  it("adds To Do task with dueDateTime", async () => {
    ...
    assert.deepStrictEqual(postStub.lastCall.args[0].data.dueDateTime, {
      dateTime: dateTime,
      timeZone: "Etc/GMT",
    });
  });
  ...
  it("adds To Do task with reminderDateTime", async () => {
    ...
    assert.deepStrictEqual(postStub.lastCall.args[0].data.reminderDateTime, {
      dateTime: dateTime,
      timeZone: "Etc/GMT",
    });
  });
  ...
  it("adds To Do task with categories ", async () => {
    ...
    assert.deepStrictEqual(postStub.lastCall.args[0].data.categories, [
      "None",
      "Preset24",
    ]);
  });
  ...
  it("adds To Do task with completedDateTime", async () => {
    ...
    assert.deepStrictEqual(postStub.lastCall.args[0].data.completedDateTime, {
      dateTime: dateTime,
      timeZone: "Etc/GMT",
    });
  });
  ...
  it("adds To Do task with startDateTime", async () => {
    ...
    assert.deepStrictEqual(postStub.lastCall.args[0].data.startDateTime, {
      dateTime: dateTime,
      timeZone: "Etc/GMT",
    });
  });
  ...
  it("handles error correctly", async () => {
    ...
    await assert.rejects(
      command.action(logger, {
        options: { listName: "Tasks List", title: "New task" },
      } as any),
