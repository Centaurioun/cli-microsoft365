...
describe(commands.TASK_LIST, () => {
    ...
    const actual = await command.validate(
      {
        options: {
          listId:
            "AQMkAGYzNjMxYTU4LTJjZjYtNDlhMi1iMzQ2LWVmMTU3YmUzOGM5MAAuAAADMN-7V4K8g0q_adetip1DygEAxMBBaLl1lk_dAn8KkjfXKQABF-BAgwAAAA==",
          listName: "Tasks List",
        },
      },
      commandInfo,
  ...
  it("fails to get ToDo Task list when the specified task list does not exist", async () => {
    sinon.stub(request, "get").callsFake((opts) => {
      if (
        (opts.url as string).indexOf(
          "/me/todo/lists?$filter=displayName eq '",
        ) > -1
      ) {
    ...
    await assert.rejects(
      command.action(logger, { options: { listName: "Tasks List" } } as any),
  ...
  it("lists To Do tasks using listName", async () => {
    sinon.stub(request, "get").callsFake((opts) => {
      if (
        ...
        return Promise.resolve({
          "@odata.context":
            "https://graph.microsoft.com/v1.0/$metadata#users('e1251b10-1ba4-49e3-b35a-933e3f21772b')/todo/lists",
          value: [
            {
      ...
      if ((opts.url as string).indexOf("/tasks") > -1) {
        return Promise.resolve({
          "@odata.context":
            "https://graph.microsoft.com/v1.0/$metadata#users('e1251b10-1ba4-49e3-b35a-933e3f21772b')/todo/lists('AQMkAGYzNjMxYTU4LTJjZjYtNDlhMi1iMzQ2LWVmMTU3YmUzOGM5MAAuAAADMN-7V4K8g0q_adetip1DygEAxMBBaLl1lk_dAn8KkjfXKQABF-BAgwAAAA%3D%3D')/tasks",
          value: [
            {
              id: "AAMkAGYzNjMxYTU4LTJjZjYtNDlhMi1iMzQ2LWVmMTU3YmUzOGM5MABGAAAAAAAw3-tXgryDSr5p162KnUPKBwDEwEFouXWWT50CfwqSN9cpAAEX8ECDAADEwEFouXWWT50CfwqSN9cpAAEX8GuPAAA=",
              title: "Stay healthy",
