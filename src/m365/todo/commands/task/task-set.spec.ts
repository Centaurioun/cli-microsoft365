...
describe(commands.TASK_SET, () => {
  ...
  const getRequestData = {
    "@odata.context":
      "https://graph.microsoft.com/v1.0/$metadata#users('4cb2b035-ad76-406c-bdc4-6c72ad403a22')/todo/lists",
    value: [
      {
  ...
  beforeEach(() => {
    ...
    patchStub = sinon.stub(request, "patch").callsFake((opts: any) => {
      if (
        opts.url ===
        "https://graph.microsoft.com/v1.0/me/todo/lists/AQMkADlhMTRkOGEzLWQ1M2QtNGVkNS04NjdmLWU0NzJhMjZmZWNmMwAuAAADKvwNgAMNPE_zFNRJXVrU1wEAhHKQZHItDEOVCn8U3xuA2AABmQeVPwAAAA==/tasks/abc"
      ) {
    ...
    sinon.stub(request, "get").callsFake((opts: any) => {
      if (
        opts.url ===
        "https://graph.microsoft.com/v1.0/me/todo/lists?$filter=displayName eq 'Tasks%20List'"
      ) {
  ...
  it("updates tasks for  list using listId", async () => {
    await command.action(logger, {
      options: {
        ...
        title: "New task",
        listId:
          "AQMkADlhMTRkOGEzLWQ1M2QtNGVkNS04NjdmLWU0NzJhMjZmZWNmMwAuAAADKvwNgAMNPE_zFNRJXVrU1wEAhHKQZHItDEOVCn8U3xuA2AABmQeVPwAAAA==",
      },
    } as any);
    assert.strictEqual(
      JSON.stringify(log[0]),
      JSON.stringify(patchRequestData),
    );
  });
  ...
  it("updates tasks for list with bodyContent and bodyContentType", async () => {
    ...
    assert.strictEqual(
      patchStub.lastCall.args[0].data.body.contentType,
      "html",
    );
  });
  ...
  it("updates tasks for list with dueDateTime", async () => {
    ...
    assert.deepStrictEqual(patchStub.lastCall.args[0].data.dueDateTime, {
      dateTime: dateTime,
      timeZone: "Etc/GMT",
    });
  });
  ...
  it("updates tasks for list with reminderDateTime", async () => {
    ...
    assert.deepStrictEqual(patchStub.lastCall.args[0].data.reminderDateTime, {
      dateTime: dateTime,
      timeZone: "Etc/GMT",
    });
  });
  ...
  it("updates To Do task with categories ", async () => {
    ...
    assert.deepStrictEqual(patchStub.lastCall.args[0].data.categories, [
      "None",
      "Preset24",
    ]);
  });
  ...
  it("updates To Do task with completedDateTime", async () => {
    ...
    assert.deepStrictEqual(patchStub.lastCall.args[0].data.completedDateTime, {
      dateTime: dateTime,
      timeZone: "Etc/GMT",
    });
  });
  ...
  it("updates To Do task with startDateTime", async () => {
    ...
    assert.deepStrictEqual(patchStub.lastCall.args[0].data.startDateTime, {
      dateTime: dateTime,
      timeZone: "Etc/GMT",
    });
  });
  ...
  it("rejects if no tasks list is found with the specified list name", async () => {
    ...
    await assert.rejects(
      ...
      new CommandError("The specified task list does not exist"),
    );
  });
  ...
  it("fails validation if status is not allowed value", async () => {
    ...
    assert.strictEqual(
      actual,
      "test is not a valid value. Allowed values are notStarted|inProgress|completed|waitingOnOthers|deferred",
    );
  });
