...
describe(commands.ENVIRONMENT_GET, () => {
  ...
  afterEach(() => {
    sinonUtil.restore([request.get, pid.getProcessName, session.getId]);
  });
  ...
  it("retrieves Microsoft Power Platform environment by name", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://api.bap.microsoft.com/providers/Microsoft.BusinessAppPlatform/environments/${formatting.encodeQueryParameter(
          environmentName,
  ...
  it("retrieves default Microsoft Power Platform environment", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://api.bap.microsoft.com/providers/Microsoft.BusinessAppPlatform/environments/~Default?api-version=2020-10-01`
      ) {
        if (
          opts.headers?.accept &&
          (opts.headers.accept as string).indexOf("application/json") === 0
  ...
  it("retrieves Microsoft Power Platform environment as Admin", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://api.bap.microsoft.com/providers/Microsoft.BusinessAppPlatform/scopes/admin/environments/${formatting.encodeQueryParameter(
          environmentName,
        )}?api-version=2020-10-01`
      ) {
        if (
