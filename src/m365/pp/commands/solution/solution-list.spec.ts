...
describe(commands.SOLUTION_LIST, () => {
  const envResponse: any = {
    properties: {
      linkedEnvironmentMetadata: {
        instanceApiUrl: "https://contoso-dev.api.crm4.dynamics.com",
      },
    },
  };
  ...
  it("retrieves solutions from power platform environment", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://api.bap.microsoft.com/providers/Microsoft.BusinessAppPlatform/environments/4be50206-9576-4237-8b17-38d8aadfaa36?api-version=2020-10-01&$select=properties.linkedEnvironmentMetadata.instanceApiUrl`
      ) {
        if (
          opts.headers?.accept &&
      ...
      if (
        opts.url ===
        `https://contoso-dev.api.crm4.dynamics.com/api/data/v9.0/solutions?$filter=isvisible eq true&$expand=publisherid($select=friendlyname)&$select=solutionid,uniquename,version,publisherid,installedon,solutionpackageversion,friendlyname,versionnumber&api-version=9.1`
      ) {
        if (
          opts.headers?.accept &&
          (opts.headers.accept as string).indexOf("application/json") === 0
    ...
    await command.action(logger, {
      options: {
        debug: true,
        environment: "4be50206-9576-4237-8b17-38d8aadfaa36",
      },
    });
  ...
  it("retrieves solutions from power platform environment in format json as admin", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://api.bap.microsoft.com/providers/Microsoft.BusinessAppPlatform/scopes/admin/environments/4be50206-9576-4237-8b17-38d8aadfaa36?api-version=2020-10-01&$select=properties.linkedEnvironmentMetadata.instanceApiUrl`
      ) {
        if (
          opts.headers?.accept &&
          (opts.headers.accept as string).indexOf("application/json") === 0
  ...
  it("correctly handles no environments", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        (opts.url as string).indexOf(
          `/providers/Microsoft.BusinessAppPlatform/environments?api-version=2020-10-01`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
    ...
    await assert.rejects(
      ...
      new CommandError(
        `The environment 'undefined' could not be retrieved. See the inner exception for more details: undefined`,
      ),
    );
  });
  ...
  it("correctly handles API OData error", async () => {
    ...
    await assert.rejects(
      command.action(logger, {
        options: { environment: "4be50206-9576-4237-8b17-38d8aadfaa36" },
      } as any),
      new CommandError(
