  ...
  it("retrieves list of gateways (debug)", async () => {
    ...
    sinon.stub(request, "get").callsFake((opts) => {
      if ((opts.url as string).indexOf(`/v1.0/myorg/gateways`) > -1) {
        if (
          opts.headers?.accept &&
          (opts.headers.accept as string).indexOf(
