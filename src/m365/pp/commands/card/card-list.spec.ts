...
describe(commands.CARD_LIST, () => {
  ...
  const cardResponse: any = {
    value: {
      "@odata.context":
        "https://contoso-dev.api.crm4.dynamics.com/api/data/v9.1/$metadata#cards(owninguser(azureactivedirectoryobjectid,fullname))",
      value: [
        {
          ...
          owninguser: {
            azureactivedirectoryobjectid:
              "88e85b64-e687-4e0b-bbf4-f42f5f8e574c",
  ...
  it("retrieves cards", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      ...
      if (
        opts.url ===
        `https://contoso-dev.api.crm4.dynamics.com/api/data/v9.1/cards?$expand=owninguser($select=azureactivedirectoryobjectid,fullname)`
      ) {
        if (
          opts.headers?.accept &&
          (opts.headers.accept as string).indexOf("application/json") === 0
