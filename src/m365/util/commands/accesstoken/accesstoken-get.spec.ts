...
describe(commands.ACCESSTOKEN_GET, () => {
  ...
  it("returns error when sharepoint specified as resource and SPO URL not available", async () => {
    ...
    await assert.rejects(
      command.action(logger, { options: { resource: "sharepoint" } } as any),
      new CommandError(
        "SharePoint URL undefined. Use the 'm365 spo set --url https://contoso.sharepoint.com' command to set the URL",
      ),
    );
  });
