...
class PaAppGetCommand extends PowerAppsCommand {
    ...
    try {
      if (args.options.name) {
        const requestOptions: CliRequestOptions = {
          url: `${
            this.resource
          }/providers/Microsoft.PowerApps/apps/${formatting.encodeQueryParameter(
            args.options.name,
        ...
        if (this.verbose) {
          logger.logToStderr(
            `Retrieving information about Microsoft Power App with name '${args.options.name}'...`,
          );
        }
        ...
        if (this.verbose) {
          logger.logToStderr(
            `Retrieving information about Microsoft Power App with displayName '${args.options.displayName}'...`,
          );
        }
        ...
        if (allApps.length > 0) {
          const app = allApps.find((a: any) => {
            return (
              a.properties.displayName.toLowerCase() ===
              `${args.options.displayName}`.toLowerCase()
            );
          });
          if (app) {
            ...
            if (this.verbose) {
              logger.logToStderr(
                `No app found with displayName '${args.options.displayName}'`,
              );
            }
          }
        } else {
  ...
  private async getApps(
    ...
    return await Cli.executeCommandWithOutput(paAppListCommand as Command, {
      options: { ...options, _: [] },
    });
  }
