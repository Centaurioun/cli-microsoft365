...
class TenantServiceAnnouncementHealthListCommand extends GraphCommand {
  ...
  private async listServiceHealth(options: Options): Promise<ServiceHealth[]> {
    const requestOptions: any = {
      url: `${this.resource}/v1.0/admin/serviceAnnouncement/healthOverviews${
        options.issues &&
        (!options.output || options.output.toLocaleLowerCase() === "json")
          ? "?$expand=issues"
          : ""
      }`,
      headers: {
    ...
    const response = await request.get<{ value: ServiceHealth[] }>(
      requestOptions,
    );
