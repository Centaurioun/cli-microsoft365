  ...
  public async commandAction(logger: Logger, args: CommandArgs): Promise<void> {
    const removeProperty: () => Promise<void> = async (): Promise<void> => {
      try {
        ...
        let identityResp = await spo.getCurrentWebIdentity(
          args.options.webUrl,
        ...
        if (opts.folder) {
          ...
          identityResp = await spo.getFolderIdentity(
  ...
  private removeProperty(
    ...
    const requestOptions: any = {
      ...
      data: `<Request AddExpandoFieldTypeSuffix="true" SchemaVersion="15.0.0.0" LibraryVersion="16.0.0.0" ApplicationName="${
        ...
        options.key,
      )}</Parameter><Parameter Type="Null" /></Parameters></Method><Method Name="Update" Id="207" ObjectPathId="198" /></Actions><ObjectPaths><Property Id="205" ParentId="198" Name="${objectType}" /><Identity Id="198" Name="${
        identityResp.objectIdentity
      }" /></ObjectPaths></Request>`,
    };
