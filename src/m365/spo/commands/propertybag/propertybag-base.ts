...
export abstract class SpoPropertyBagBaseCommand extends SpoCommand {
   ...
   */
  protected getFolderPropertyBag(
    identityResp: IdentityResponse,
    webUrl: string,
    folder: string,
    logger: Logger,
  ): Promise<any> {
    ...
    return new Promise<any>((resolve: any, reject: any) => {
      return request.post<string>(requestOptions).then(
        (res: string) => {
          ...
          const propertiesObj = json.find((x) => {
            return x["Properties"];
          });
          if (propertiesObj) {
    ...
    return new Promise<any>((resolve: any, reject: any): void => {
      request.post(requestOptions).then(
        (res: any) => {
          ...
          const allPropertiesObj = json.find((x) => {
            return x["AllProperties"];
          });
          if (allPropertiesObj) {
  ...
  public static setProperty(
    ...
    const requestOptions: any = {
      ...
      }" xmlns="http://schemas.microsoft.com/sharepoint/clientquery/2009"><Actions><Method Name="SetFieldValue" Id="206" ObjectPathId="205"><Parameters><Parameter Type="String">${formatting.escapeXml(
        name,
      )}</Parameter><Parameter Type="String">${formatting.escapeXml(
        value,
      )}</Parameter></Parameters></Method><Method Name="Update" Id="207" ObjectPathId="198" /></Actions><ObjectPaths><Property Id="205" ParentId="198" Name="${objectType}" /><Identity Id="198" Name="${
        identityResp.objectIdentity
      }" /></ObjectPaths></Request>`,
    };
   ...
   */
  public static isNoScriptSite(
    webUrl: string,
    formDigest: string,
    webIdentityResp: IdentityResponse,
    logger: Logger,
    debug: boolean,
  ): Promise<boolean> {
    return new Promise<boolean>(
      (
        resolve: (isNoScriptSite: boolean) => void,
        reject: (error: any) => void,
      ): void => {
        spo
          .getEffectiveBasePermissions(
            webIdentityResp.objectIdentity,
            webUrl,
            formDigest,
            logger,
            debug,
          ...
          .then((basePermissionsResp: BasePermissions): void => {
            resolve(
              basePermissionsResp.has(PermissionKind.AddAndCustomizePages) ===
                false,
            );
          })
