...
describe(commands.LIST_CONTENTTYPE_ADD, () => {
  ...
  afterEach(() => {
    sinonUtil.restore([request.post, cli.getSettingWithDefaultValue]);
  });
  ...
  it("adds content type to the specific list if listTitle option is passed (debug)", async () => {
    sinon.stub(request, "post").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://contoso.sharepoint.com/sites/dummy/_api/web/lists/getByTitle('Documents')/ContentTypes/AddAvailableContentType`
      ) {
        if (
          opts.headers?.accept &&
  ...
  it("adds content type to the specific list if listId option is passed (debug)", async () => {
    sinon.stub(request, "post").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://contoso.sharepoint.com/sites/dummy/_api/web/lists(guid'dfddade1-4729-428d-881e-7fedf3cae50d')/ContentTypes/AddAvailableContentType`
      ) {
        if (
          opts.headers?.accept &&
          (opts.headers.accept as string).indexOf("application/json") === 0 &&
  ...
  it("command correctly handles list get reject request", async () => {
    ...
    sinon.stub(request, "post").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://contoso.sharepoint.com/_api/web/lists/getByTitle('Documents')/ContentTypes/AddAvailableContentType`
      ) {
        throw err;
      }
  ...
  it("uses correct API url when listId option is passed", async () => {
    sinon.stub(request, "post").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://contoso.sharepoint.com/_api/web/lists(guid'dfddade1-4729-428d-881e-7fedf3cae50d')/ContentTypes/AddAvailableContentType`
      ) {
        return response;
      }
  ...
  it("adds contenttype to list when list url is used", async () => {
    sinon.stub(request, "post").callsFake(async (opts) => {
      if (
        opts.url ===
        "https://contoso.sharepoint.com/_api/web/GetList('%2Fsites%2Fdocuments')/ContentTypes/AddAvailableContentType"
      ) {
        return response;
      }
  ...
  it("fails validation if both listId and listTitle options are not passed", async () => {
    const actual = await command.validate(
      { options: { webUrl: "https://contoso.sharepoint.com", id: "0x0120" } },
      commandInfo,
    );
