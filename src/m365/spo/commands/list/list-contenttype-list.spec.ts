...
describe(commands.LIST_CONTENTTYPE_LIST, () => {
  ...
  it("retrieves all content types of the specific list if listTitle option is passed (debug)", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://contoso.sharepoint.com/sites/ninja/_api/web/lists/getByTitle('Documents')/ContentTypes`
      ) {
        if (
          opts.headers?.accept &&
    ...
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        (opts.url as string).indexOf(
          `https://contoso.sharepoint.com/sites/ninja/_api/web/lists(guid'dfddade1-4729-428d-881e-7fedf3cae50d')/ContentTypes`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
  ...
  it("retrieves all content types of the specific list if listUrl option is passed", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        "https://contoso.sharepoint.com/_api/web/GetList('%2Fsites%2Fdocuments')/ContentTypes"
      ) {
        if (
          opts.headers?.accept &&
          (opts.headers.accept as string).indexOf("application/json") === 0
  ...
  it("command correctly handles list get reject request", async () => {
    ...
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://contoso.sharepoint.com/_api/web/lists/getByTitle('Documents')/ContentTypes`
      ) {
        throw err;
      }
