...
describe(commands.LIST_VIEW_SET, () => {
  ...
  it("fails validation if webUrl is not a valid SharePoint URL", async () => {
    const actual = await command.validate(
      {
        options: { webUrl: "invalid", listTitle: listTitle, title: viewTitle },
      },
      commandInfo,
    );
  ...
  it("fails validation if listId is not a GUID", async () => {
    const actual = await command.validate(
      { options: { webUrl: webUrl, listId: "invalid", title: viewTitle } },
      commandInfo,
    );
    assert.notStrictEqual(actual, true);
  });
  ...
  it("fails validation if id is not a GUID", async () => {
    const actual = await command.validate(
      { options: { webUrl: webUrl, listTitle: "List 1", id: "invalid" } },
      commandInfo,
    );
    assert.notStrictEqual(actual, true);
  });
  ...
  it("passes validation when id and listId specified as valid GUIDs", async () => {
    const actual = await command.validate(
      { options: { webUrl: webUrl, listId: listId, id: viewId } },
      commandInfo,
    );
  ...
  it("ignores global options when creating request data", async () => {
    const patchRequest: sinon.SinonStub = sinon
      .stub(request, "patch")
      .callsFake(async (opts) => {
        if (
          opts.url ===
          `${webUrl}/_api/web/lists/GetByTitle('${formatting.encodeQueryParameter(
            listTitle,
          )}')/views/GetByTitle('${formatting.encodeQueryParameter(
            viewTitle,
          )}')`
        ) {
          if (
            ...
            JSON.stringify(opts.data) ===
              JSON.stringify({ Title: "All events" })
          ) {
            return;
          }
        }
    ...
    assert.deepEqual(patchRequest.lastCall.args[0].data, {
      Title: "All events",
    });
  });
  ...
  it("updates the Title of the list view specified using its name", async () => {
    sinon.stub(request, "patch").callsFake(async (opts) => {
      if (
        ...
        `${webUrl}/_api/web/lists/GetByTitle('${formatting.encodeQueryParameter(
          listTitle,
        )}')/views/GetByTitle('${formatting.encodeQueryParameter(viewTitle)}')`
      ) {
        if (
          opts.headers?.accept &&
          (opts.headers.accept as string).indexOf("application/json") === 0 &&
  ...
  it("updates the Title and CustomFormatter of the list view specified using its ID", async () => {
    sinon.stub(request, "patch").callsFake(async (opts) => {
      if (
        opts.url ===
        `${webUrl}/_api/web/lists(guid'${formatting.encodeQueryParameter(
          listId,
        )}')/views/GetById('${formatting.encodeQueryParameter(viewId)}')`
      ) {
        if (
          opts.headers?.accept &&
          (opts.headers.accept as string).indexOf("application/json") === 0 &&
          opts.headers["X-RequestDigest"] &&
          JSON.stringify(opts.data) ===
            JSON.stringify({ Title: "All events", CustomFormatter: "abc" })
        ) {
          return;
        }
      }
  ...
  it("updates the Title and CustomFormatter of the list view specified using its Url", async () => {
    sinon.stub(request, "patch").callsFake(async (opts) => {
      const serverRelativeUrl: string = urlUtil.getServerRelativePath(
        webUrl,
        listUrl,
      );
      ...
      if (
        opts.url ===
        `${webUrl}/_api/web/GetList('${formatting.encodeQueryParameter(
          serverRelativeUrl,
        )}')/views/GetById('${formatting.encodeQueryParameter(viewId)}')`
      ) {
        if (
