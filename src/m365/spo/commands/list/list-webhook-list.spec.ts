...
describe(commands.LIST_WEBHOOK_LIST, () => {
    ...
    value: [
      {
        ...
        notificationUrl:
          "https://deletemetestfunction.azurewebsites.net/api/FakeWebhookEndpoint?code=QlM2zaeJRti4WFGQUEqSo1ZmKMtRdB2JQ3mc2kzPj2aX6pNBAWVU4w==",
    ...
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        (opts.url as string).indexOf(
          `https://contoso.sharepoint.com/sites/ninja/_api/web/lists/GetByTitle('Documents')/Subscriptions`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
  ...
  it("retrieves all webhooks of the specific list if listId option is passed", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        (opts.url as string).indexOf(
          `https://contoso.sharepoint.com/sites/ninja/_api/web/lists(guid'dfddade1-4729-428d-881e-7fedf3cae50d')/Subscriptions`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
  ...
  it("fails validation if the listId option is not a valid GUID", async () => {
    const actual = await command.validate(
      {
        options: { webUrl: "https://contoso.sharepoint.com", listId: "12345" },
      },
      commandInfo,
    );
    ...
    const actual = await command.validate(
      {
        options: {
          webUrl: "notavalidurl",
          listId: "0CD891EF-AFCE-4E55-B836-FCE03286CCC",
        },
      },
      commandInfo,
    );
