...
class SpoFolderRenameCommand extends SpoCommand {
  ...
  public async commandAction(logger: Logger, args: CommandArgs): Promise<void> {
    try {
      ...
      const webIdentityResp = await spo.getCurrentWebIdentity(
        args.options.webUrl,
        formDigestValue,
      );
      const folderObjectIdentity = await spo.getFolderIdentity(
      ...
      if (this.verbose) {
        logger.logToStderr(
          `Renaming folder ${args.options.url} to ${args.options.name}`,
        );
      }
      ...
      const serverRelativeUrlWithoutOldFolder: string =
        serverRelativePath.substring(0, serverRelativePath.lastIndexOf("/"));
