...
describe(commands.THEME_APPLY, () => {
  ...
  it("applies theme when correct parameters are passed", async () => {
    const postStub: sinon.SinonStub = sinon
      .stub(request, "post")
      .callsFake((opts) => {
        if (
            ...
            JSON.stringify([
              {
                ...
                ErrorInfo: null,
                TraceCorrelationId: "3d92299e-e019-4000-c866-de7d45aa9628",
              },
              12,
              true,
            ]),
    ...
    assert.strictEqual(
      postStub.lastCall.args[0].headers["X-RequestDigest"],
      "ABC",
      "request digest",
    );
    assert.strictEqual(
      postStub.lastCall.args[0].data,
  ...
  it("applies SharePoint theme (Blue) when correct parameters are passed", async () => {
    sinon.stub(request, "post").callsFake((opts) => {
      ...
      if ((opts.url as string).indexOf(`/_api/ThemeManager/ApplyTheme`) > -1) {
        return Promise.resolve(
          JSON.stringify({
            "@odata.context":
              "https://contoso.sharepoint.com/sites/project-x/_api/$metadata#Edm.String",
    ...
    requests.forEach((r) => {
      if (r.url.indexOf(`/_api/ThemeManager/ApplyTheme`) > -1 && r.data) {
      ...
      if (
        ...
        return Promise.resolve(
          JSON.stringify([
            {
              ErrorInfo: {
                ErrorMessage: "requestObjectIdentity ClientSvc error",
              },
            },
          ]),
        );
  ...
  it("handles unknown error command error correctly", async () => {
    sinon.stub(request, "post").callsFake((opts) => {
      if (
        ...
        return Promise.resolve(
          JSON.stringify([
            { ErrorInfo: { ErrorMessage: "ClientSvc unknown error" } },
          ]),
        );
      }
      return Promise.reject("Invalid request");
            ...
            JSON.stringify([
              {
                SchemaVersion: "15.0.0.0",
                LibraryVersion: "16.0.7025.1207",
                ErrorInfo: "{ErrorMessage:error occurred}",
                TraceCorrelationId: "3d92299e-e019-4000-c866-de7d45aa9628",
              },
              12,
              false,
  ...
  it("fails validation if webUrl is not passed", async () => {
    const actual = await command.validate(
      { options: { name: "Contoso-Blue", webUrl: "" } },
      commandInfo,
    );
  ...
  it("fails validation if webUrl is not a valid SharePoint URL", async () => {
    const actual = await command.validate(
      { options: { name: "Contoso-Blue", webUrl: "invalid" } },
      commandInfo,
    );
    assert.notStrictEqual(actual, true);
  });
