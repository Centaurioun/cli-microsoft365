...
describe(commands.STORAGEENTITY_SET, () => {
  ...
  afterEach(() => {
    sinonUtil.restore([request.post, spo.getSpoAdminUrl]);
  });
  ...
  it("sets tenant property", async () => {
    ...
    assert.strictEqual(
      (postStub.lastCall.args[0].headers as any)["X-RequestDigest"],
      "ABC",
    );
    assert.strictEqual(
  ...
  it("correctly handles access denied error when setting tenant property", async () => {
    ...
    assert.strictEqual(
      loggerLogToStderrSpy.calledWithMatch(
        "This error is often caused by invalid URL of the app catalog site",
      ),
      true,
    );
  });
  ...
  it("rejects invalid SharePoint Online URL", async () => {
    ...
    const actual = await command.validate(
      { options: { appCatalogUrl: url, key: "prop", value: "val" } },
      commandInfo,
    );
    assert.strictEqual(
      actual,
      `${url} is not a valid SharePoint Online site URL`,
    );
  });
