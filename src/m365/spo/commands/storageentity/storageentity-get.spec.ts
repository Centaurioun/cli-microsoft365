    ...
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        (opts.url as string).indexOf(
          `/_api/web/GetStorageEntity('existingproperty')`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
      ...
      if (
        (opts.url as string).indexOf(
          `/_api/web/GetStorageEntity('propertywithoutcomments')`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
      ...
      if (
        (opts.url as string).indexOf(
          `/_api/web/GetStorageEntity('nonexistingproperty')`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
