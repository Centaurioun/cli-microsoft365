...
describe(commands.CDN_POLICY_SET, () => {
  ...
  it("sets IncludeFileExtensions CDN policy on the public CDN when no type specified", async () => {
    await command.action(logger, {
      options: { debug: true, policy: "IncludeFileExtensions", value: "WOFF" },
    });
    let setRequestIssued = false;
    requests.forEach((r) => {
      if (
  ...
  it("sets ExcludeRestrictedSiteClassifications CDN policy on the public CDN when no type specified", async () => {
    await assert.rejects(
      command.action(logger, {
        options: {
          policy: "ExcludeRestrictedSiteClassifications",
          value: "foo",
        },
      }),
    );
  ...
  it("escapes XML in user input", async () => {
    ...
    await command.action(logger, {
      options: { policy: "IncludeFileExtensions", value: "<WOFF" },
    });
  ...
  it("rejects invalid SharePoint Online CDN type", async () => {
    ...
    assert.strictEqual(
      actual,
      `${type} is not a valid CDN type. Allowed values are Public|Private`,
    );
  });
  ...
  it("rejects invalid SharePoint Online CDN policy", async () => {
    const policy = "foo";
    const actual = await command.validate(
      { options: { policy: policy, value: "bar" } },
      commandInfo,
    );
    assert.strictEqual(
      actual,
      `${policy} is not a valid CDN policy. Allowed values are IncludeFileExtensions|ExcludeRestrictedSiteClassifications`,
    );
  });
});
