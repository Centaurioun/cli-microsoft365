...
describe(commands.CDN_ORIGIN_ADD, () => {
            ...
            return JSON.stringify([
              {
                ...
                ErrorInfo: null,
                TraceCorrelationId: "a05d299e-0036-4000-8546-cfc42dc07fd2",
              },
              42,
              [
                "*\u002fMASTERPAGE",
  ...
  it("sets CDN origin on the public CDN when Public type specified", async () => {
    await command.action(logger, {
      options: { debug: true, origin: "*/cdn", type: "Public" },
    });
  ...
  it("sets CDN origin on the private CDN when Private type specified", async () => {
    await assert.rejects(
      command.action(logger, {
        options: { debug: true, origin: "*/cdn", type: "Private" },
      }),
    );
    let setRequestIssued = false;
    requests.forEach((r) => {
            ...
            return JSON.stringify([
              {
                ...
                ErrorInfo: {
                  ErrorMessage:
                    "The library is already registered as a CDN origin.",
                  ErrorValue: null,
  ...
  it("accepts Public SharePoint Online CDN type", async () => {
    const actual = await command.validate(
      { options: { type: "Public", origin: "*/CDN" } },
      commandInfo,
    );
  ...
  it("accepts Private SharePoint Online CDN type", async () => {
    const actual = await command.validate(
      { options: { type: "Private", origin: "*/CDN" } },
      commandInfo,
    );
    assert.strictEqual(actual, true);
  });
