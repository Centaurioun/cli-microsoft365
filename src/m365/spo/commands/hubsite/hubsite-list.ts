...
class SpoHubSiteListCommand extends SpoCommand {
  ...
  public async commandAction(logger: Logger, args: CommandArgs): Promise<void> {
    try {
      ...
      const hubSitesResult = await odata.getAllItems<HubSite>(
        `${spoAdminUrl}/_api/hubsites`,
      );
      const hubSites = hubSitesResult;

      if (
        !(
          args.options.includeAssociatedSites !== true ||
        ...
        const requestOptions: CliRequestOptions = {
          ...
          data: {
            parameters: {
              ViewXml:
                '<View><Query><Where><And><And><IsNull><FieldRef Name="TimeDeleted"/></IsNull><Neq><FieldRef Name="State"/><Value Type=\'Integer\'>0</Value></Neq></And><Neq><FieldRef Name="HubSiteId"/><Value Type=\'Text\'>{00000000-0000-0000-0000-000000000000}</Value></Neq></And></Where><OrderBy><FieldRef Name=\'Title\' Ascending=\'true\' /></OrderBy></Query><ViewFields><FieldRef Name="Title"/><FieldRef Name="SiteUrl"/><FieldRef Name="SiteId"/><FieldRef Name="HubSiteId"/></ViewFields><RowLimit Paged="TRUE">' +
                this.batchSize +
                "</RowLimit></View>",
              DatesInUtc: true,
            },
        ...
        if (this.debug) {
          logger.logToStderr(
            `Will retrieve associated sites (including the hub sites) in batches of ${this.batchSize}`,
          );
        }

        const res = await this.getSites(
          requestOptions,
        ...
        if (res) {
          hubSites.forEach((h) => {
            const filteredSites = res.filter((f) => {
              ...
              return (
                f.SiteId !== f.HubSiteId &&
                (f.HubSiteId as string).toUpperCase() ===
                  `{${h.ID.toUpperCase()}}`
              );
            });
  ...
  private async getSites(
    ...
    nonPagedUrl: string,
    logger: Logger,
    sites: AssociatedSite[] = [],
    batchNumber: number = 0,
  ): Promise<AssociatedSite[]> {
    const res = await request.post<QueryListResult>(reqOptions);
    batchNumber++;
    const retrievedSites: AssociatedSite[] =
      res.Row.length > 0 ? sites.concat(res.Row) : sites;

    if (this.debug) {
      logger.logToStderr(res);
      logger.logToStderr(
        `Retrieved ${res.Row.length} sites in batch ${batchNumber}`,
      );
    }
