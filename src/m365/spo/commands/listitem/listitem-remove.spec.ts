...
describe(commands.LISTITEM_REMOVE, () => {
  ...
  it("removes the list item when prompt confirmed", async () => {
    ...
    requests.forEach((r) => {
      if (
        r.url.indexOf(`/_api/web/lists(guid'`) > -1 &&
        r.headers.accept &&
  ...
  it("removes the list item from list retrieved by listUrl when prompt confirmed", async () => {
    sinon.stub(request, "post").callsFake(async (opts) => {
      ...
      if (
        opts.url ===
          `https://contoso.sharepoint.com/sites/project-x/_api/web/GetList('${formatting.encodeQueryParameter(
    ...
    sinon.stub(Cli, "prompt").callsFake(async () => ({ continue: true }));
    await command.action(logger, {
      options: { verbose: true, listUrl: listUrl, webUrl: webUrl, id: 1 },
    });
    let correctRequestIssued = false;
    requests.forEach((r) => {
      if (
  ...
  it("fails validation if both id and title options are not passed", async () => {
    const actual = await command.validate(
      { options: { webUrl: "https://contoso.sharepoint.com", id: 1 } },
      commandInfo,
    );
  ...
  it("fails validation if the url option is not a valid SharePoint site URL", async () => {
    const actual = await command.validate(
      { options: { webUrl: "foo", id: 1, listTitle: "Documents" } },
      commandInfo,
    );
    assert.notStrictEqual(actual, true);
  });
