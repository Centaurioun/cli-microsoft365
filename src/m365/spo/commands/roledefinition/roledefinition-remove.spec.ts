...
describe(commands.ROLEDEFINITION_REMOVE, () => {
  ...
  afterEach(() => {
    sinonUtil.restore([request.delete, Cli.prompt]);
  });
  ...
  it("fails validation if id is not a number", async () => {
    const actual = await command.validate(
      { options: { webUrl: "https://contoso.sharepoint.com", id: "abc" } },
      commandInfo,
    );
    assert.notStrictEqual(actual, true);
  });
  ...
  it("removes the role definition when prompt confirmed", async () => {
    ...
    await command.action(logger, {
      options: { debug: true, webUrl: "https://contoso.sharepoint.com", id: 1 },
    });
    let correctRequestIssued = false;
    requests.forEach((r) => {
      if (
        r.url.indexOf(`/_api/web/roledefinitions(1)`) > -1 &&
        r.headers.accept &&
