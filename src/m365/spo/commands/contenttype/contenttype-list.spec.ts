...
describe(commands.CONTENTTYPE_LIST, () => {
    ...
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        (opts.url as string).indexOf(
          `https://contoso.sharepoint.com/sites/test/_api/web/ContentTypes`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
  ...
  it("retrieves all content types by category (debug)", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        (opts.url as string).indexOf(
          `https://contoso.sharepoint.com/sites/test/_api/web/ContentTypes?$filter=Group eq 'List%20Content%20Types'`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
