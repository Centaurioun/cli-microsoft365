...
describe(commands.APP_INSTALL, () => {
    ...
    sinon.stub(request, "post").callsFake(async (opts) => {
      ...
      if (
        (opts.url as string).indexOf(
          `/_api/web/tenantappcatalog/AvailableApps/GetById('b2307a39-e878-458b-bc90-03bc578531d6')/install`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
  ...
  it("installs app in the specified site from site collection app catalog", async () => {
    sinon.stub(request, "post").callsFake(async (opts) => {
      ...
      if (
        (opts.url as string).indexOf(
          `/_api/web/sitecollectionappcatalog/AvailableApps/GetById('b2307a39-e878-458b-bc90-03bc578531d6')/install`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
