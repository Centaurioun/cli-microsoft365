...
describe(commands.APP_ADD, () => {
  ...
  it("adds new app to the tenant app catalog", async () => {
    sinon.stub(request, "post").callsFake(async (opts) => {
      if (
        (opts.url as string).indexOf(
          `/_api/web/tenantappcatalog/Add(overwrite=false, url='spfx.sppkg')`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
    ...
    await command.action(logger, {
      options: { filePath: "spfx.sppkg", output: "text" },
    });
  ...
  it("adds new app to the tenant app catalog (debug)", async () => {
    ...
    try {
      await command.action(logger, {
        options: { debug: true, filePath: "spfx.sppkg" },
      });
      ...
      sinonUtil.restore([request.post, fs.readFileSync]);
    }
  });
  ...
  it("adds new app to a site app catalog (debug)", async () => {
    sinon.stub(request, "post").callsFake(async (opts) => {
      ...
      if (
        (opts.url as string).indexOf(
          `/_api/web/sitecollectionappcatalog/Add(overwrite=false, url='spfx.sppkg')`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
  ...
  it("returns all info about the added app in the JSON output mode", async () => {
    ...
    try {
      await command.action(logger, {
        options: { filePath: "spfx.sppkg", output: "json" },
      });
      assert(
            ...
            error: JSON.stringify({
              "odata.error": {
                ...
                message: {
                  lang: "en-US",
                  value:
                    "A file with the name AppCatalog/spfx.sppkg already exists. It was last modified by i:0#.f|membership|admin@contoso.onmi on 24 Nov 2017 12:50:43 -0800.",
                },
              },
            }),
          };
        ...
        return JSON.stringify([
          {
            ...
            ErrorInfo: {
              ...
              ErrorCode: -1,
              ErrorTypeName: "Microsoft.SharePoint.Client.ServerException",
            },
            TraceCorrelationId: "18091989-62a6-4cad-9717-29892ee711bc",
          },
        ]);
  ...
  it("passes validation on valid 'tenant' scope", async () => {
    ...
    const actual = await command.validate(
      { options: { appCatalogScope: "tenant", filePath: "abc" } },
      commandInfo,
    );
    sinonUtil.restore([fs.existsSync, fs.lstatSync]);
  ...
  it("submits to tenant app catalog when scope 'tenant' specified ", async () => {
    ...
    try {
      await command.action(logger, {
        options: { appCatalogScope: "tenant", filePath: "spfx.sppkg" },
      });
      let correctAppCatalogUsed = false;
      requests.forEach((r) => {
        if (r.url.indexOf("/tenantappcatalog/") > -1) {
  ...
  it("fails validation when invalid scope is specified", async () => {
    ...
    const actual = await command.validate(
      { options: { filePath: "abc", appCatalogScope: "foo" } },
      commandInfo,
    );
  ...
  it("passes validation when the scope is specified with 'tenant'", async () => {
    ...
    const actual = await command.validate(
      { options: { filePath: "abc", appCatalogScope: "tenant" } },
      commandInfo,
    );
  ...
  it("should fail when 'sitecollection' scope, but no appCatalogUrl specified", async () => {
    ...
    const actual = await command.validate(
      { options: { filePath: "abc", appCatalogScope: "sitecollection" } },
      commandInfo,
    );
