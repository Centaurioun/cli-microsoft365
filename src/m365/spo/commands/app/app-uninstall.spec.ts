...
describe(commands.APP_UNINSTALL, () => {
    ...
    sinon.stub(request, "post").callsFake(async (opts) => {
      ...
      if (
        (opts.url as string).indexOf(
          `/_api/web/tenantappcatalog/AvailableApps/GetById('b2307a39-e878-458b-bc90-03bc578531d6')/uninstall`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
  ...
  it("uninstalls app from the specified site installed from the site collection app catalog", async () => {
    ...
    sinon.stub(request, "post").callsFake(async (opts) => {
      ...
      if (
        (opts.url as string).indexOf(
          `/_api/web/sitecollectionappcatalog/AvailableApps/GetById('b2307a39-e878-458b-bc90-03bc578531d6')/uninstall`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
  ...
  it("fails validation when the id is not a valid GUID", async () => {
    const actual = await command.validate(
      { options: { id: "123", siteUrl: "https://contoso.sharepoint.com" } },
      commandInfo,
    );
  ...
  it("fails validation if the siteUrl option is not a valid SharePoint site URL", async () => {
    const actual = await command.validate(
      {
        options: { id: "b2307a39-e878-458b-bc90-03bc578531d6", siteUrl: "foo" },
      },
      commandInfo,
    );
    assert.notStrictEqual(actual, true);
