...
describe(commands.APP_DEPLOY, () => {
    ...
    sinon.stub(request, "post").callsFake(async (opts) => {
      ...
      if (
        (opts.url as string).indexOf(
          `/_api/web/tenantappcatalog/AvailableApps/GetById('b2307a39-e878-458b-bc90-03bc578531d6')/deploy`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
  ...
  it("deploys app in the sitecollection app catalog", async () => {
    ...
    sinon.stub(request, "post").callsFake(async (opts) => {
      ...
      if (
        (opts.url as string).indexOf(
          `/_api/web/sitecollectionappcatalog/AvailableApps/GetById('b2307a39-e878-458b-bc90-03bc578531d6')/deploy`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
  ...
  it("deploys app specified using its name in the tenant app catalog", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      ...
      if (
        (opts.url as string).indexOf(
          `/_api/web/getfolderbyserverrelativeurl('AppCatalog')/files('solution.sppkg')?$select=UniqueId`,
        ) > -1
      ) {
  ...
  it("deploys app specified using its name in the site app catalog (debug)", async () => {
    ...
    sinon.stub(request, "post").callsFake(async (opts) => {
      ...
      if (
        (opts.url as string).indexOf(
          `https://contoso.sharepoint.com/_api/web/sitecollectionappcatalog/AvailableApps/GetById('b2307a39-e878-458b-bc90-03bc578531d6')/deploy`,
        ) > -1
      ) {
        if (
          opts.headers?.accept &&
      ...
      requests.forEach((r) => {
        if (
          ...
          JSON.stringify(r.data) ===
            JSON.stringify({ skipFeatureDeployment: true })
        ) {
          correctRequestIssued = true;
        }
      });
    ...
    try {
      ...
      requests.forEach((r) => {
        if (
          ...
          r.headers.accept.indexOf("application/json") === 0 &&
          JSON.stringify(r.data) ===
            JSON.stringify({ skipFeatureDeployment: false })
        ) {
          correctRequestIssued = true;
        }
      });
  ...
  it("fails validation when the scope is specified invalid option", async () => {
    const actual = await command.validate(
      { options: { name: "solution", appCatalogScope: "foo" } },
      commandInfo,
    );
    assert.notStrictEqual(actual, true);
  });
  ...
  it("should fail when 'sitecollection' scope, but no appCatalogUrl specified", async () => {
    const actual = await command.validate(
      { options: { name: "solution", appCatalogScope: "sitecollection" } },
      commandInfo,
    );
    assert.notStrictEqual(actual, true);
  });
