...
describe("Telemetry", () => {
  ...
  it("doesn't log an event when disableTelemetry is set", async () => {
    sinon
      .stub(Cli.getInstance(), "getSettingWithDefaultValue")
      .callsFake((settingName, defaultValue) => {
  ...
  it("doesn't log an exception when disableTelemetry is set", async () => {
    ...
    const exceptionStub = sinon
      .stub(appInsights, "trackException")
      .callsFake(() => {});
  ...
  it("logs an exception when disableTelemetry is not set", async () => {
    ...
    const trackExceptionStub = sinon
      .stub(appInsights, "trackException")
      .callsFake(() => {});
