...
describe("utils/cache", () => {
  ...
  describe("clearExpired", () => {
    ...
    it("skips directories while clearing expired entries (dir + file)", (done) => {
      sinon
        .stub(fs, "readdir")
        .callsArgWith(1, undefined, ["directory", "file"]);
      ...
      try {
        cache.clearExpired(() => {
          try {
            assert(
              unlinkStub.calledWith(path.join(cache.cacheFolderPath, "file")),
            );
            done();
          } catch (ex) {
            done(ex);
    ...
    it("doesn't fail while removing expired cache entry fails", (done) => {
      ...
      const twoDaysAgo = new Date();
      twoDaysAgo.setDate(twoDaysAgo.getDate() - 2);
      sinon.stub(fs, "stat").callsArgWith(1, undefined, {
        isDirectory: () => false,
        atime: twoDaysAgo,
      });
      const unlinkStub = sinon
