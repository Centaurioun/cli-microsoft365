...
describe("utils/md", () => {
  ...
  before(() => {
    cliCompletionClinkUpdateHelp = fs.readFileSync(
      path.join(
        __dirname,
        "..",
        "..",
        ...
        "cli",
        "completion",
        "completion-clink-update.mdx",
      ),
      "utf8",
    );
    cliCompletionClinkUpdateHelpPlain = md.md2plain(
      cliCompletionClinkUpdateHelp,
      path.join(__dirname, "..", "..", "docs"),
    );
    loginHelp = fs.readFileSync(
      path.join(__dirname, "..", "..", "docs", "docs", "cmd", "login.mdx"),
      "utf8",
    );
    loginHelpPlain = md.md2plain(
      loginHelp,
      path.join(__dirname, "..", "..", "docs"),
    );
  });
  ...
  it("converts title to uppercase", () => {
    assert(
      cliCompletionClinkUpdateHelpPlain.includes("CLI COMPLETION CLINK UPDATE"),
    );
  });
  ...
  it("converts definition lists", () => {
    assert(
      cliCompletionClinkUpdateHelpPlain.includes(
  ...
  it("keeps only label when hyperlink label and URL are the same", () => {
    assert(
      cliCompletionClinkUpdateHelpPlain.includes(
        "https://pnp.github.io/cli-microsoft365/user-guide/completion/",
      ),
    );
    assert(
      !cliCompletionClinkUpdateHelpPlain.includes(
