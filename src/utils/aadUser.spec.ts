...
describe("utils/aadUser", () => {
  ...
  it("correctly get user id by upn", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://graph.microsoft.com/v1.0/users?$filter=userPrincipalName eq '${formatting.encodeQueryParameter(
          validUserName,
        )}'&$select=Id`
      ) {
        return userResponse;
  ...
  it("correctly gets a single user by Email.", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://graph.microsoft.com/v1.0/users?$filter=mail eq '${formatting.encodeQueryParameter(
          validUserName,
        )}'&$select=id`
      ) {
        return userResponse;
  ...
  it("throws error message when no user was found with a specific upn", async () => {
    ...
    await assert.rejects(
      aadUser.getUserIdByUpn(validUserName),
      Error(
        `The specified user with user name ${validUserName} does not exist.`,
      ),
    );
  });
  ...
  it("throws error message when no user was found with a specific mail", async () => {
    ...
    await assert.rejects(
      aadUser.getUserIdByEmail(validUserName),
      Error(`The specified user with email ${validUserName} does not exist`),
    );
  });
  ...
  it("correctly get upn by user id", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://graph.microsoft.com/v1.0/users/${validUserId}?$select=userPrincipalName`
      ) {
