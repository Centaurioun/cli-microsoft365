...
export const validation = {
  ...
  isValidTeamsChannelId(guid: string): boolean {
    const guidRegEx: RegExp = new RegExp(
      /^19:[0-9a-zA-Z-_]+@thread\.(skype|tacv2)$/i,
    );
  ...
  isValidTeamsChatId(guid: string): boolean {
    const guidRegEx: RegExp = new RegExp(
      /^19:[0-9a-zA-Z-_]+(@thread\.v2|@unq\.gbl\.spaces)$/i,
    );
  ...
  isJavaScriptReservedWord(input: string): boolean {
    ...
    return (
      ...
      !input
        .split(".")
        .every((value) => !~javascriptReservedWords.indexOf(value))
    );
  },
  ...
  isValidFileName(input: string): boolean {
    return (
      !!input &&
      !/^((\..*)|COM\d|CLOCK\$|LPT\d|AUX|NUL|CON|PRN|(.*[\u{d800}-\u{dfff}]+.*))$/iu.test(
        input,
      ) &&
      !/^(.*\.\..*)$/i.test(input)
    );
  ...
  isValidTheme(input: string): boolean {
    ...
    const hasInvalidProperties = validThemeProperties
      .map((property) => {
    ...
    const hasInvalidValues = validThemeProperties
      .map((property: string) => {
