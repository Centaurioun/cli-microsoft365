...
const stubPostResponses: any = (folderAddResp: any = null) => {
  return sinon.stub(request, "post").callsFake((opts) => {
    if (
      (opts.url as string).indexOf("/_api/web/GetFolderByServerRelativePath") >
      -1
    ) {
      if (folderAddResp) {
        ...
        return Promise.resolve({
          ...
          Name: "4t4",
          ProgID: null,
          ServerRelativeUrl: "/sites/JohnDoe/Shared Documents/4t4",
          TimeCreated: "2018-10-26T22:50:27Z",
          TimeLastModified: "2018-10-26T22:50:27Z",
          UniqueId: "3f5428e2-b0a8-4d35-87df-89621ed5b457",
          WelcomePage: "",
        });
      }
    }
...
const stubGetResponses: any = (
  ...
  return sinon.stub(request, "get").callsFake((opts) => {
    if (
      (opts.url as string).indexOf("/_api/web/GetFolderByServerRelativeUrl(") >
      -1
    ) {
      if (getFolderByServerRelativeUrlResp) {
        ...
        return Promise.resolve({
          ...
          Name: "f",
          ProgID: null,
          ServerRelativeUrl: "/sites/JohnDoe/Shared Documents/4t4/f",
          TimeCreated: "2018-10-26T22:54:19Z",
          TimeLastModified: "2018-10-26T22:54:20Z",
          UniqueId: "0d680f20-53da-4516-b3f6-ed98b1d928e8",
          WelcomePage: "",
        });
      }
    }
...
describe("utils/spo", () => {
  ...
  it("handles error when retrieving SPO URL failed while retrieving tenant app catalog url", (done) => {
    ...
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        (opts.url as string).indexOf("/_api/SP_TenantSettings_Current") > -1
      ) {
    ...
    spo.getTenantAppCatalogUrl(logger, false).then(
      () => {
        done("Expected error");
      },
      (err: string) => {
  ...
  it("retrieves SPO URL from MS Graph when not retrieved previously", (done) => {
    ...
    spo.getSpoUrl(logger, false).then((spoUrl: string) => {
      try {
  ...
  it("retrieves SPO URL from MS Graph when not retrieved previously (debug)", (done) => {
    ...
    spo.getSpoUrl(logger, true).then((spoUrl: string) => {
      try {
        assert.strictEqual(spoUrl, "https://contoso.sharepoint.com");
        done();
      } catch (e) {
  ...
  it("returns error when retrieving SPO URL failed", (done) => {
    ...
    spo.getSpoUrl(logger, false).then(
      () => {
        done("Expected error");
      },
      (err: string) => {
  ...
  it("returns error when retrieving SPO admin URL failed", (done) => {
    sinon
      .stub(spo, "getSpoUrl")
      .callsFake(() => Promise.reject("An error has occurred"));

    spo.getSpoAdminUrl(logger, false).then(
      () => {
        done("Expected error");
      },
      (err: string) => {
  ...
  it("retrieves tenant ID when not retrieved previously", (done) => {
    ...
    sinon
      .stub(spo, "getSpoAdminUrl")
      .callsFake(() => Promise.resolve("https://contoso-admin.sharepoint.com"));
    ...
    spo.getTenantId(logger, false).then((tenantId: string) => {
      try {
  ...
  it("retrieves tenant ID when not retrieved previously (debug)", (done) => {
    ...
    spo.getTenantId(logger, true).then((tenantId: string) => {
      try {
        assert.strictEqual(tenantId, "tenantId");
        done();
      } catch (e) {
  ...
  it("returns error when retrieving tenant ID failed", (done) => {
    ...
    spo.getTenantId(logger, false).then(
      () => {
        done("Error expected");
      },
      (err: any) => {
  ...
  it("should reject if wrong url param", (done) => {
    spo.ensureFolder("abc", "abc", logger, true).then(
      () => {
  ...
  it("should reject if empty folder param", (done) => {
    spo.ensureFolder("https://contoso.sharepoint.com", "", logger, true).then(
      () => {
        done("Should reject, not resolve");
      },
      (err: any) => {
  ...
  it("should handle folder creation failure", (done) => {
    const folderDoesNotExistErrorResp: any = new Promise<any>(
      (resolve, reject) => {
        return reject(
          JSON.stringify({
            "odata.error": {
              code: "-2130575338, Microsoft.SharePoint.SPException",
              message: { lang: "en-US", value: "Error: Not found." },
            },
          }),
        );
      },
    ...
    const expectedError = JSON.stringify({
      "odata.error": {
        code: "-2130575338, Microsoft.SharePoint.SPException",
        message: { lang: "en-US", value: "Error: Cannot create folder." },
      },
    });
  ...
  it("should succeed in adding folder if it does not exist (debug)", (done) => {
    ...
    spo
      ...
      .then(
        () => {
          assert.strictEqual(
            loggerLogSpy.lastCall.args[0],
            "All sub-folders exist",
          );
          done();
        },
        (err: any) => {
  ...
  it("should have the correct url when calling AddSubFolderUsingPath (POST)", (done) => {
    ...
    spo
      ...
      .then(
        () => {
          assert.strictEqual(
            postStubs.lastCall.args[0].url,
            "https://contoso.sharepoint.com/_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/AddSubFolderUsingPath(DecodedUrl=@a2)?@a1=%27%2Ffolder2%27&@a2=%27folder3%27",
          );
          done();
        },
        (err: any) => {
    ...
    spo
      ...
      .then(
        () => {
          assert.strictEqual(
            postStubs.lastCall.args[0].url,
            "https://contoso.sharepoint.com/sites/Site1/_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/AddSubFolderUsingPath(DecodedUrl=@a2)?@a1=%27%2Fsites%2FSite1%2Ffolder2%27&@a2=%27folder3%27",
          );
          done();
        },
        (err: any) => {
  ...
  it("should call two times AddSubFolderUsingPath when folderUrl is folder2/folder3", (done) => {
    ...
    spo
      ...
      .then(
        () => {
          assert.strictEqual(
            postStubs.getCall(0).args[0].url,
            "https://contoso.sharepoint.com/sites/Site1/_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/AddSubFolderUsingPath(DecodedUrl=@a2)?@a1=%27%2Fsites%2FSite1%27&@a2=%27folder2%27",
          );
          assert.strictEqual(
            postStubs.getCall(1).args[0].url,
            "https://contoso.sharepoint.com/sites/Site1/_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/AddSubFolderUsingPath(DecodedUrl=@a2)?@a1=%27%2Fsites%2FSite1%2Ffolder2%27&@a2=%27folder3%27",
          );
          done();
        },
    ...
    spo
      ...
      .then(
        () => {
          assert.strictEqual(
            postStubs.getCall(0).args[0].url,
            "https://contoso.sharepoint.com/sites/site1/_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/AddSubFolderUsingPath(DecodedUrl=@a2)?@a1=%27%2Fsites%2Fsite1%27&@a2=%27PnP1%27",
          );
          assert.strictEqual(
            postStubs.getCall(1).args[0].url,
            "https://contoso.sharepoint.com/sites/site1/_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/AddSubFolderUsingPath(DecodedUrl=@a2)?@a1=%27%2Fsites%2Fsite1%2FPnP1%27&@a2=%27Folder2%27",
          );
          done();
        },
        (err: any) => {
  ...
  it("should call GetFolderByServerRelativeUrl with the correct url OData values", (done) => {
    ...
    spo
      ...
      .then(
        () => {
          assert.strictEqual(
            getStubs.getCall(0).args[0].url,
            "https://contoso.sharepoint.com/sites/Site1/_api/web/GetFolderByServerRelativeUrl('%2Fsites%2FSite1%2Ffolder2')",
          );
          assert.strictEqual(
            getStubs.getCall(1).args[0].url,
            "https://contoso.sharepoint.com/sites/Site1/_api/web/GetFolderByServerRelativeUrl('%2Fsites%2FSite1%2Ffolder2%2Ffolder3')",
          );
          done();
        },
        (err: any) => {
  ...
  const customActionOnSiteResponse1 = {
    ClientSideComponentId: "d1e5e0d6-109d-40c4-a53e-924073fe9bbd",
    ClientSideComponentProperties: '{"testMessage":"Test message"}',
    CommandUIExtension: null,
    Description: null,
    Group: null,
    Id: "a6c7bef2-42d5-405c-a89f-6e36b3c302b3",
    ImageUrl: null,
  ...
  const customActionOnSiteResponse2 = {
    ClientSideComponentId: "230edcf5-2df5-480f-9707-ae1118726912",
    ClientSideComponentProperties: '{"testMessage":"Test message"}',
    CommandUIExtension: null,
    Description: null,
    Group: null,
    Id: "06d3eebb-6e30-4346-aecd-f84a342a9316",
    ImageUrl: null,
    Location: "ClientSideExtension.ApplicationCustomizer",
    Name: "YourName",
    RegistrationId: null,
  ...
  const customActionOnWebResponse1 = {
    ...
    Description: null,
    Group: null,
    Id: "8b86123a-3194-49cf-b167-c044b613a48a",
    ImageUrl: null,
    Location: "ClientSideExtension.ApplicationCustomizer",
    Name: "YourName",
    RegistrationId: null,
  ...
  const customActionOnWebResponse2 = {
    ClientSideComponentId: "a405a600-7a21-49e7-9964-5e8b010b9eec",
    ClientSideComponentProperties: '{"testMessage":"Test message"}',
    CommandUIExtension: null,
    Description: null,
    Group: null,
    Id: "9115bb61-d9f1-4ed4-b7b7-e5d1834e60f5",
    ImageUrl: null,
    Location: "ClientSideExtension.ApplicationCustomizer",
    Name: "YourName",
    RegistrationId: null,
  ...
  const customActionsOnSiteResponse = [
    customActionOnSiteResponse1,
    customActionOnSiteResponse2,
  ];
  const customActionsOnWebResponse = [
    customActionOnWebResponse1,
    customActionOnWebResponse2,
  ];
  //#endregion
  ...
  it("returns a list of custom actions with scope 'All'", async () => {
    sinon.stub(request, "get").callsFake((opts) => {
      if ((opts.url as string).indexOf("/_api/Site/UserCustomActions") > -1) {
        ...
        (opts.url as string).indexOf("/_api/Web/UserCustomActions") > -1
      ) {
    ...
    const customActions = await spo.getCustomActions(
  ...
  it("returns a list of custom actions with scope 'Web' with a filter", async () => {
    sinon.stub(request, "get").callsFake((opts) => {
      if (
        (opts.url as string).indexOf(
          "/_api/Web/UserCustomActions?$filter=ClientSideComponentId eq guid'b41916e7-e69d-467f-b37f-ff8ecf8f99f2'",
        ) > -1
      ) {
    ...
    const customActions = await spo.getCustomActions(
      "https://contoso.sharepoint.com/sites/sales",
      "Web",
      "ClientSideComponentId eq guid'b41916e7-e69d-467f-b37f-ff8ecf8f99f2'",
    );
  ...
  it("retrieves a custom action by id with scope 'All'", async () => {
    sinon.stub(request, "get").callsFake((opts) => {
      if (
        (opts.url as string).indexOf(
          "/_api/Site/UserCustomActions(guid'd1e5e0d6-109d-40c4-a53e-924073fe9bbd')",
        ) > -1
      ) {
        return Promise.resolve(customActionOnSiteResponse1);
      } else if (
        (opts.url as string).indexOf(
          "/_api/Web/UserCustomActions(guid'd1e5e0d6-109d-40c4-a53e-924073fe9bbd')",
        ) > -1
      ) {
    ...
    const customAction = await spo.getCustomActionById(
      "https://contoso.sharepoint.com/sites/sales",
      "d1e5e0d6-109d-40c4-a53e-924073fe9bbd",
    );
  ...
  it("retrieves Azure AD ID by SPO user ID sucessfully", async () => {
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        "https://contoso.sharepoint.com/sites/sales/_api/web/siteusers/GetById('9')?$select=AadObjectId"
      ) {
        return {
          AadObjectId: {
    ...
    const customAction = await spo.getUserAzureIdBySpoId(
      "https://contoso.sharepoint.com/sites/sales",
      "9",
    );
  ...
  it("retrieves spo user by email sucessfully", async () => {
    ...
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://contoso.sharepoint.com/sites/sales/_api/web/siteusers/GetByEmail('${formatting.encodeQueryParameter(
    ...
    const user = await spo.getUserByEmail(
      "https://contoso.sharepoint.com/sites/sales",
      "john.doe@contoso.com",
      logger,
      true,
  ...
  it("throws error retrieving a custom action by id with a wrong scope value", async () => {
    try {
      ...
      assert.deepEqual(
        e,
        "Invalid scope 'Invalid'. Allowed values are 'Site', 'Web' or 'All'.",
      );
    }
  });
  ...
  const topNavigationResponse = {
    ...
    Nodes: [
        ...
        Nodes: [
          {
            ...
            SimpleUrl: "/sites/PnPCoreSDKTestGroup",
            Title: "Sub level 1",
            Translations: [],
          },
        ],
      ...
      },
      {
        ...
        NodeType: 0,
        OpenInNewWindow: true,
        SimpleUrl: "/sites/team-a/sitepages/about.aspx",
        Title: "About",
        Translations: [],
      },
    ],
    SimpleUrl: "",
    SPSitePrefix: "/sites/SharePointDemoSite",
    SPWebPrefix: "/sites/SharePointDemoSite",
    StartingNodeKey: "1025",
    StartingNodeTitle: "Quick launch",
    Version: "2023-03-09T18:33:53.5468097Z",
  };
  const quickLaunchResponse = {
    AudienceIds: [],
    FriendlyUrlPrefix: "",
    IsAudienceTargetEnabledForGlobalNav: false,
    Nodes: [
        ...
        Nodes: [
          {
            ...
            NodeType: 0,
            OpenInNewWindow: null,
            SimpleUrl: "/sites/SharePointDemoSite#/",
            Title: "Sub Item",
            Translations: [],
          },
        ],
    ...
    Version: "2023-03-09T18:34:53.650545Z",
  };
  ...
  it("retrieves spo group by name sucessfully", async () => {
    ...
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        `https://contoso.sharepoint.com/sites/sales/_api/web/sitegroups/GetByName('${formatting.encodeQueryParameter(
          "groupname",
        )}')`
      ) {
    ...
    const group = await spo.getGroupByName(
      "https://contoso.sharepoint.com/sites/sales",
      "groupname",
      logger,
      true,
  ...
  it("retrieves roledefinition by name sucessfully", async () => {
    ...
    sinon.stub(request, "get").callsFake(async (opts) => {
      if (
        opts.url ===
        "https://contoso.sharepoint.com/sites/sales/_api/web/roledefinitions"
      ) {
    ...
    const roledefintion = await spo.getRoleDefinitionByName(
      "https://contoso.sharepoint.com/sites/sales",
      "Read",
      logger,
      true,
