...
class Request {
  ...
  public set debug(debug: boolean) {
    ...
    if (this._debug) {
      this.req.interceptors.request.use(
        (config: CliRequestOptions): CliRequestOptions => {
          if (this._logger) {
            ...
            this._logger.logToStderr(
              JSON.stringify(
                formatting.filterObject(config, properties),
                null,
                2,
              ),
            );
      ...
      (
        ...
        ((): Promise<string> => {
          if (options.headers?.["x-anonymous"]) {
            ...
            const url = options.headers?.["x-resource"]
              ? options.headers["x-resource"]
              : options.url;
          ...
          .then(
            (res: any): void => {
              if (resolve) {
                resolve(
                  options.responseType === "stream" || options.fullResponse
                    ? res
                    : res.data,
                );
              } else {
            ...
            (error: AxiosError): void => {
              if (
                ...
                if (this._debug) {
                  (this._logger as Logger).log(
                    `Request throttled. Waiting ${retryAfter}sec before retrying...`,
                  );
                }
                setTimeout(() => {
  ...
  private updateRequestForCloudType(
