...
console.log(
  'Deprecate beta versions of the @pnp/cli-microsoft365 npm package on npm'
)
...
const allVersions = JSON.parse(
  await $`npm view @pnp/cli-microsoft365 versions -json`
)
const versionsToDeprecate = allVersions.filter(
  (v) => v !== null && v.startsWith(`${version}-beta`)
)
